...
import { HealthIndicator, HealthStatus } from '@/components/ui/health-status';
import { EventSeverityBadge, RecentEvents, UsageMetrics } from '@/components/ui/monitoring-metrics';
import AppLayout from '@/layouts/app-layout';
import { adminRoutes } from '@/lib/admin-routes';
import { type BreadcrumbItem, type OAuthClient, type OAuthClientEvent, type OAuthClientUsage } from '@/types';
...
export default function OAuthClientShow({
    client,
    usage = [],
    events = [],
    stats = {
    ...
    const currentBreadcrumbs = [...breadcrumbs, { title: client.name, href: adminRoutes.oauthClients.show(client.id) }];
    ...
    return (
        <AppLayout breadcrumbs={currentBreadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col gap-6 overflow-x-auto rounded-xl p-6">
                ...
                <div className="flex items-start justify-between">
                    <div className="space-y-1">
                        <div className="flex items-center gap-3">
                            ...
                            <HealthStatus status={client.health_status} size="lg" />
                        ...
                        {client.description && <p className="text-lg text-muted-foreground">{client.description}</p>}
                    ...
                    <div className="flex items-center gap-2">
                        <Button variant="outline" onClick={handleToggleStatus} className={client.is_active ? 'text-red-600 dark:text-red-400' : 'text-green-600 dark:text-green-400'}>
                            {client.is_active ? <PauseCircle className="mr-2 h-4 w-4" /> : <PlayCircle className="mr-2 h-4 w-4" />}
                            {client.is_active ? 'Deactivate' : 'Activate'}
                        </Button>

                        <Button variant="outline" onClick={handleToggleMaintenance}>
                ...
                {clientSecret && (
                    <Card className="border-blue-200 bg-blue-50 dark:border-blue-700 dark:bg-blue-950/50">
                        <CardHeader>
                            ...
                            <CardDescription className="text-blue-600 dark:text-blue-400">
                                Save this secret now! It will only be shown once for security reasons.
                            </CardDescription>
                        </CardHeader>
                        <CardContent>
                            <div className="flex items-center gap-2 rounded border bg-white dark:bg-gray-800 dark:border-gray-600 p-3">
                                <code className="flex-1 font-mono text-sm break-all text-gray-900 dark:text-gray-100">{clientSecret}</code>
                                <Button
                                    variant="outline"
                                    size="sm"
                                    ...
                                    className="shrink-0"
                                >
                                    {copiedField === 'client_secret' ? <CheckCircle className="h-4 w-4" /> : <Copy className="h-4 w-4" />}
                                </Button>
                            </div>
                            <p className="mt-2 text-xs text-blue-600 dark:text-blue-400">
                                Store this secret securely. You can regenerate it later if needed, but it will invalidate any existing integrations.
                            </p>
                        </CardContent>
                    </Card>
                )}
                    ...
                    <TabsContent value="overview" className="space-y-6">
                        <div className="grid gap-6 md:grid-cols-2">
                                ...
                                <CardContent className="space-y-4">
                                    <div>
                                        ...
                                        <div className="mt-1 flex items-center gap-2">
                                            <code className="flex-1 rounded bg-muted px-2 py-1 font-mono text-sm">{client.identifier}</code>
                                            <Button variant="ghost" size="sm" onClick={() => copyToClipboard(client.identifier, 'client_id')}>
                                    ...
                                    {client.contact_email && (
                                        <div>
                                            ...
                                            <div className="mt-1 text-sm">
                                                <a href={`mailto:${client.contact_email}`} className="text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300">
                                                    {client.contact_email}
                                                </a>
                                            </div>
                                        </div>
                            ...
                            <Card>
                                ...
                                <CardContent className="space-y-4">
                                    <HealthIndicator status={client.health_status} lastCheckedAt={client.last_health_check_at} />
                                    ...
                                    <div className="grid grid-cols-2 gap-4 text-sm">
                                        ...
                                        <div>
                                            ...
                                            <div className="text-2xl font-bold text-green-600 dark:text-green-400">{stats?.active_tokens || 0}</div>
                                        </div>
                                        <div>
                                ...
                                <CardContent className="space-y-4">
                                    <div>
                                        ...
                                        <div className="mt-1 space-y-1">
                                            {(Array.isArray(client.redirect_uris) ? client.redirect_uris : []).map((uri, index) => (
                                                <code key={index} className="block rounded bg-muted px-2 py-1 font-mono text-sm">
                                                    {uri}
                                                </code>
                                            ))}
                                        </div>
                                    ...
                                    <div>
                                        ...
                                        <div className="mt-1 flex flex-wrap gap-1">
                                            {(Array.isArray(client.grants) ? client.grants : []).map((grant) => (
                                ...
                                <CardContent className="space-y-4">
                                    ...
                                    {client.health_check_enabled && client.health_check_url && (
                                        <div>
                                            ...
                                            <code className="mt-1 block rounded bg-muted px-2 py-1 font-mono text-sm">{client.health_check_url}</code>
                                            <div className="mt-1 text-xs text-muted-foreground">Interval: {client.health_check_interval} seconds</div>
                                        </div>
                                    )}
                                </CardContent>
                            </Card>
                    ...
                    <TabsContent value="monitoring" className="space-y-6">
                        <Card>
                            <CardHeader>
                                ...
                                <CardDescription>Historical usage data and performance metrics</CardDescription>
                            </CardHeader>
                            <CardContent>
                                {usage.length > 0 ? (
                                        ...
                                        {usage.slice(0, 7).map((dayUsage) => (
                                            <div key={dayUsage.id} className="border-b pb-4">
                                                <div className="mb-2 flex items-center justify-between">
                                                    ...
                                                    <span className="text-sm text-muted-foreground">{dayUsage.api_calls} API calls</span>
                                                </div>
                                                <div className="grid grid-cols-4 gap-4 text-sm">
                                                    ...
                                                    <div>
                                                        ...
                                                        <div className="font-semibold text-red-600 dark:text-red-400">{dayUsage.error_count}</div>
                                                    </div>
                                                </div>
                                            </div>
                                        ))}
                    ...
                    <TabsContent value="events" className="space-y-4">
                        <Card>
                            <CardHeader>
                                ...
                                <CardDescription>Complete log of client events and activities</CardDescription>
                            </CardHeader>
                            <CardContent>
                                {events.length > 0 ? (
                                            ...
                                            <div key={event.id} className="flex items-start justify-between rounded border p-3">
                                                <div className="flex-1">
                                                    ...
                                                    {event.event_description && (
                                                        <p className="mb-2 text-sm text-muted-foreground">{event.event_description}</p>
                                                    )}
                                                    {event.ip_address && (
                                                        <span className="text-xs text-muted-foreground">From {event.ip_address}</span>
                                                    )}
                                                </div>
                    ...
                    <TabsContent value="tokens">
                        <Card>
                            <CardHeader>
                                ...
                                <CardDescription>Manage active tokens for this client</CardDescription>
                            </CardHeader>
                            <CardContent>
                                <div className="mb-4 flex items-center justify-between">
                                    <div>
                                        <div className="text-2xl font-bold">{stats?.active_tokens || 0}</div>
                                ...
                                <Button asChild variant="outline" className="w-full">
                                    <Link href={`${adminRoutes.tokens.index()}?client_id=${client.id}`}>View All Tokens</Link>
                                </Button>
                            </CardContent>
                        </Card>
                    </TabsContent>
                        ...
                        <AlertDialog>
                            ...
                            <AlertDialogContent>
                                <AlertDialogHeader>
                                    ...
                                    <AlertDialogDescription>
                                        Are you sure you want to delete <strong>{client.name}</strong>? This will revoke all associated tokens and
                                        cannot be undone.
                                    </AlertDialogDescription>
                                </AlertDialogHeader>
                                <AlertDialogFooter>
